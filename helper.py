...
def create_manifest(slug, plugin_dir="."):
    ...
    manifest['version'] = input_default(
        "Version", manifest.get('version', "1.0.0"))
    manifest['license'] = input_default(
        "License (if open-source, use license identifier from https://spdx.org/licenses/)", manifest.get('license', "proprietary"))
...
def create_module(slug, panel_filename=None, source_filename=None):
    ...
    if module_manifest:
        ...
        tags = input_default(
            "Tags (comma-separated, case-insensitive, see https://github.com/VCVRack/Rack/blob/v1/src/plugin.cpp#L511-L571 for list)")
        tags = tags.split(",")
...
def panel_to_components(tree):
    ...
    width_mult = float(root.get('width')[:-2]) / float(viewBox[2])
    height_mult = float(root.get('height')[:-2]) / float(viewBox[3])
    ...
    colors = {
        'param': ['red', '#ff0000', '#f00'],
        'input': ['green', 'lime', '#00ff00', '#0f0'],
        'output': ['blue', '#0000ff', '#00f'],
        'light': ['magenta', '#ff00ff', '#f0f'],
        'widget': ['yellow', '#ffff00', '#ff0']
    }
    ...
    for el in circles + rects:
        ...
        if el.tag == "{http://www.w3.org/2000/svg}rect":
            x = width_mult * float(el.get('x'))
            y = height_mult * float(el.get('y'))
            width = width_mult * float(el.get('width'))
            height = height_mult * float(el.get('height'))
            c['x'] = round(x, 3)
            c['y'] = round(y, 3)
            ...
            cx = width_mult * float(el.get('cx'))
            cy = height_mult * float(el.get('cy'))
            c['cx'] = round(cx, 3)
            c['cy'] = round(cy, 3)

        if color in colors['param']:
            components['params'].append(c)
        if color in colors['input']:
            components['inputs'].append(c)
        if color in colors['output']:
            components['outputs'].append(c)
        if color in colors['light']:
            components['lights'].append(c)
        if color in colors['widget']:
            components['widgets'].append(c)

    # Sort components
    def top_left_sort(w): return (w['cy'], w['cx'])
